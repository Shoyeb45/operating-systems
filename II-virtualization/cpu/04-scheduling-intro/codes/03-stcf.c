...
void simulate_cpu_execution(int idx, long *current_time, int *proc_completed) {
    ...
    *current_time = *current_time + 1;
    ...
    if (processes[idx].remaining_time == 0) {
        ...
        *proc_completed = *proc_completed + 1;
    }
}
    ...
    while (process_completed < total_processes) {
        ...
        for (int i = 0; i < total_processes; i++) {
            if (!processes[i].completed && processes[i].arrival_time <= current_time) {
                if (shortest_idx == -1 || processes[i].remaining_time < processes[shortest_idx].remaining_time) {
                    shortest_idx = i;
                }
            }
        }
        ...
        simulate_cpu_execution(shortest_idx, &current_time, &process_completed);
    }
}
...
void usage() {
    fprintf(stderr, "usage: ./sjb <arrival_time job_time>[]\nEx., ./sjb 0 100 10 10 10 10\nIt means, we have P1 - "
                    "arrived at 0 with job time 100 and so on.");
    exit(1);
}
